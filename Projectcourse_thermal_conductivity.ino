...
float set_thickness  = 7;        // [m]
float set_T_cool     = 40.0;      // [C] 
float set_T_hot      = 50.0;      // [C]
...
void loop() 
{
  ...
  float temp_cool = temp_sensor_cool.getTempCByIndex(0);
  float temp_hot  = temp_sensor_hot.getTempCByIndex(0);
  
  // Kalibrering av temperatursensorer
  read_T_cool =  temp_sensor_cool.getTempCByIndex(0);
  ...
  //---------------------------------------------
  // Regulation of peltier, turns off when tempeture goes higher than the set temperature.
  if (read_T_cool <= set_T_cool) {  
  ...
  // Safty regulation of heatfoil, turns of when tempeture goes lower than 5 degrees.
  if (read_T_hot <= 5) { 
    digitalWrite(relay_peltier, LOW);
  ...
  k_value = (q_value * set_thickness_m)/(read_T_hot - read_T_cool);
...
{  
  ...
  lcd.print(q_value); //counter (0 to 3)
